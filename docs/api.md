...
World clocks keyed by their qualified `minecraft:world_clock` registry names (26.1+). Each clock has `totalTicks` (bigint), `partialTick` (number, defaults to zero) and `rate` (number). They advance using elapsed time independently of physics and are cleared when entering configuration. The dimension's `default_clock` selects the public time fields; unknown clocks leave those fields `null`.
...
This function returns a `Promise`, with `void` as its argument after sending the close and attempting an inventory synchronization on 1.16.5 and below. Modern versions resolve after sending the packet; this is not a server acknowledgement barrier.
