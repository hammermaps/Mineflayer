...
module.exports = () => async (bot) => {
  ...
  // Establish a daytime transition first; a stale night packet cannot then
  // satisfy the midnight command's acknowledgement.
  const daylight = onceWithCleanup(bot, 'time', { timeout: 5000, checkCondition: () => bot.time.timeOfDay < 1000 })
  bot.chat('/time set 0')
  await daylight
  const night = onceWithCleanup(bot, 'time', { timeout: 5000, checkCondition: () => bot.time.timeOfDay >= midnight && bot.time.timeOfDay < midnight + 1000 })
