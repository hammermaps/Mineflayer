...
// 26.3 changed light masks from big-endian i64 pairs to ByteBuf BitSet bytes.
// prismarine-chunk accepts [most-significant, least-significant] int32 pairs.
function normalizeLightMask (mask) {
  if (!Buffer.isBuffer(mask)) return mask
  const longs = []
  for (let offset = 0; offset < mask.length; offset += 8) {
    let low = 0
    let high = 0
    for (let byte = 0; byte < 4 && offset + byte < mask.length; byte++) low |= mask[offset + byte] << (byte * 8)
    for (let byte = 0; byte < 4 && offset + 4 + byte < mask.length; byte++) high |= mask[offset + 4 + byte] << (byte * 8)
    longs.push([high | 0, low | 0])
  }
  return longs
}
...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  function addColumn (args) {
    ...
    try {
      ...
      if (args.skyLight !== undefined) {
        column.loadParsedLight(args.skyLight, args.blockLight, normalizeLightMask(args.skyLightMask), normalizeLightMask(args.blockLightMask), normalizeLightMask(args.emptySkyLightMask), normalizeLightMask(args.emptyBlockLightMask))
      }
  ...
  bot._client.on('update_light', (packet) => {
    ...
    if (bot.supportFeature('newLightingDataFormat')) {
      column.loadParsedLight(packet.skyLight, packet.blockLight, normalizeLightMask(packet.skyLightMask), normalizeLightMask(packet.blockLightMask), normalizeLightMask(packet.emptySkyLightMask), normalizeLightMask(packet.emptyBlockLightMask))
    } else {
