...
function inject (bot) {
  ...
  bot._client.on('rel_entity_move', (packet) => {
    ...
    const delta = packet.delta
      ? (Array.isArray(packet.delta)
          ? packet.delta.reduce((total, step) => ({ dX: total.dX + step.dX, dY: total.dY + step.dY, dZ: total.dZ + step.dZ }), { dX: 0, dY: 0, dZ: 0 })
          : packet.delta)
      : packet
    if (bot.supportFeature('fixedPointDelta')) {
      entity.position.translate(delta.dX / 32, delta.dY / 32, delta.dZ / 32)
    } else if (bot.supportFeature('fixedPointDelta128')) {
      entity.position.translate(delta.dX / (128 * 32), delta.dY / (128 * 32), delta.dZ / (128 * 32))
    }
  ...
  bot._client.on('entity_teleport', (packet) => {
    ...
    if (packet.dx !== undefined) {
      ...
      const yawDegrees = packet.yaw + (flags.yaw ? oldYaw : 0)
      const pitchDegrees = Math.max(-90, Math.min(90, packet.pitch + (flags.pitch ? oldPitch : 0)))
      const yaw = conv.fromNotchianYaw(yawDegrees)
      const pitch = conv.fromNotchianPitch(pitchDegrees)
      let { x, y, z } = entity.velocity
      if (flags.yawDelta) {
        const pitchDelta = (oldPitch - pitchDegrees) * Math.PI / 180
        const yawDelta = (oldYaw - yawDegrees) * Math.PI / 180
        const rotatedY = y * Math.cos(pitchDelta) + z * Math.sin(pitchDelta)
        z = z * Math.cos(pitchDelta) - y * Math.sin(pitchDelta)
        y = rotatedY
        const rotatedX = x * Math.cos(yawDelta) + z * Math.sin(yawDelta)
        z = z * Math.cos(yawDelta) - x * Math.sin(yawDelta)
        x = rotatedX
      }
      entity.position.set(packet.x + (flags.x ? entity.position.x : 0), packet.y + (flags.y ? entity.position.y : 0), packet.z + (flags.z ? entity.position.z : 0))
      entity.velocity.set(packet.dx + (flags.dx ? x : 0), packet.dy + (flags.dy ? y : 0), packet.dz + (flags.dz ? z : 0))
      entity.yaw = yaw
      entity.pitch = pitch
  ...
  bot._client.on('sync_entity_position', (packet) => {
    ...
    if (bot.registry.version['>=']('26.3')) {
      const path = Array.isArray(packet.positionPath) ? packet.positionPath.at(-1) : packet.positionPath
      if (!path) return
      entity.position.set(path.x, path.y, path.z)
      entity.yaw = conv.fromNotchianYaw(packet.yRot)
      entity.pitch = conv.fromNotchianPitch(packet.xRot)
      entity.onGround = packet.onGround
      bot.emit('entityMoved', entity)
      return
    }
  ...
  function swingArm (arm = 'right', showHand = true) {
    ...
    const packet = {}
    if (showHand && !bot.registry.version['>=']('26.3')) packet.hand = hand
