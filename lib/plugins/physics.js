...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function tickPhysics (now) {
    ...
    if (shouldUsePhysics) {
      updatePosition(now)
      // 26.3 resets its one-position-packet guard only when the client
      // completes a tick. Without this acknowledgement Paper rejects the
      // second position update as invalid player movement.
      if (bot.registry.protocol?.play?.toServer?.types?.packet_tick_end) {
  ...
  function updatePosition (now) {
    // In 26.3 accepting a teleport itself is a position change on the server.
    // Do not send a second position packet in that same server tick.
    if (now < nextPositionUpdateAt) return
    ...
    const positionUpdated = lastSent.x !== position.x || lastSent.y !== position.y || lastSent.z !== position.z ||
      ...
      (!bot.registry.version['>=']('26.3') && (Math.round((now - lastSent.time) / PHYSICS_INTERVAL_MS) * PHYSICS_INTERVAL_MS) >= 1000)
  ...
  bot._client.on('position', (packet) => {
    ...
    if (bot.supportFeature('teleportUsesOwnPacket')) {
      const confirmation = { teleportId: packet.teleportId }
      ...
      bot._client.write('teleport_confirm', confirmation)
      // The acknowledgement itself updates the server position in 26.3.  Hold
      // movement for one server tick only: a one-second pause lets pathfinder
      // accumulate several blocks into its first move packet, which Paper
      // rightfully rejects as invalid player movement.
      if (bot.registry.version['>=']('26.3')) nextPositionUpdateAt = performance.now() + PHYSICS_INTERVAL_MS
    }
    ...
    if (respawnTimer > 0 && Date.now() - respawnTimer < 2000 && !bot.registry.version['>=']('26.3')) {
    ...
    // 26.3 carries this acknowledgement in teleport_confirm. Sending the
    // legacy forced movement immediately afterwards is a second position in
    // the same server tick and Vanilla disconnects it as invalid.
    if (!bot.registry.version['>=']('26.3')) sendPacketPositionAndLook(pos, newYaw, newPitch, bot.entity.onGround)
    else {
      lastSent.x = pos.x
      lastSent.y = pos.y
      lastSent.z = pos.z
