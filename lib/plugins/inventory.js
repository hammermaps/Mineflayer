...
function inject (bot, { hideErrors }) {
  ...
  async function activateEntityAt (entity, position) {
    ...
    bot._client.write('use_entity', {
      ...
      location: position.minus(entity.position)
    })
  }
  ...
  function closeWindow (window) {
    ...
    // close_window restores the server's player-inventory window (id 0).
    // Synchronize that active window, rather than clicking the closed container.
  ...
  bot._ensureHasSentCarriedItem = () => {
    ...
    bot._client.write('held_item_slot', { slotId: bot.quickBarSlot })
    carriedIndex = bot.quickBarSlot
  }
  bot._client.on('login', () => { carriedIndex = 0 })
  bot._client.on('held_item_slot', (packet) => {
    if (!Number.isInteger(packet.slot) || packet.slot < 0 || packet.slot >= 9) return
    const changed = packet.slot !== bot.quickBarSlot
    // An authoritative server selection is already synchronized: never echo it.
    carriedIndex = packet.slot
