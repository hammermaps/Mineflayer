...
export async function validateDataInstall (packageRoot, source, destination, inherited) {
  ...
  for (const entry of entries) {
    ...
    if (JSON.stringify(JSON.parse(target)) !== JSON.stringify(JSON.parse(expected))) {
      ...
      const isReviewed263Migration = entry.name === 'protocol.json' && source.endsWith('/26.3') && destination.endsWith('/26.3')
      // Exact reviewed historical datasets only; permit one-way migration.
      if (!isReviewed263Migration && (entry.name !== 'protocol.json' || !['d512c599f778c61f1effe99fd8f3db8d49f01dc5cc823c0b39ecf5acda05bd93', '07808c04b4c2379069e5d5ef84bf81b48d16f65d7c1092596ee78376b65782e4', '1eda5d35a5820538e15a67220ef59bebe1ecbe7a8e5078d4a1db3474bbdc1e87'].includes(hash))) {
